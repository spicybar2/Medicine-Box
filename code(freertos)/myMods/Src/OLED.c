...
/**
  * @brief  在OLED显示一个16x16汉字
  * @param  x: 起始横坐标 (0~127)
  * @param  y: 起始页坐标 (0~7，每页8像素高)
  * @param  Chinese: 要显示的汉字字符串 (如 "已")
  */
void OLED_ShowChinese(uint8_t x, uint8_t y, char *Chinese)
{
	uint8_t i, j;
	uint8_t font_count = sizeof(MyFont) / sizeof(Chinese_Font); // 自动计算字库大小

	for (i = 0; i < font_count; i++)
	{
		// 查找匹配的汉字索引
		if (strcmp(MyFont[i].Index, Chinese) == 0)
		{
			// 1. 发送上半部分 (前16字节)
			OLED_SetCursor(y, x);
			for (j = 0; j < 16; j++)
			{
				OLED_WriteData(MyFont[i].Msk[j]);
			}
